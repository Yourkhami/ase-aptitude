...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ================================================================
  // 8. INTERACTIVE DEMO CLASS FORM VALIDATION & WHATSAPP REDIRECT
  // ================================================================
  ...
  if (demoForm) {
    demoForm.addEventListener('submit', (e) => {
      ...
      if (!courseVal) {
        courseError.textContent = 'Please choose a course of interest.';
        isValid = false;
      }

      if (!isValid) return;

      const batchVal = timeInput ? timeInput.value : 'Flexible';
      const msgVal = messageInput ? messageInput.value.trim() : '';

      // Save inquiry to localStorage for Admin Panel access
      try {
        const storedLeads = JSON.parse(localStorage.getItem('ase_registrations') || '[]');
        storedLeads.unshift({
          name: nameVal,
          phone: phoneVal,
          course: courseVal,
        ...
        localStorage.setItem('ase_registrations', JSON.stringify(storedLeads));
      } catch (err) {
        console.warn('Could not persist registration to storage:', err);
      }
      ...
      const whatsappText = encodeURIComponent(
        ...
        `📚 *Course:* ${courseVal}\n` +
        `⏰ *Preferred Time:* ${batchVal}\n` +
        (msgVal ? `💬 *Notes:* ${msgVal}\n` : '') +
        `\nPlease confirm my demo class slot.`
      );

      const whatsappUrl = `https://wa.me/919525301743?text=${whatsappText}`;

      formFeedback.innerHTML = `
        <div style="display: flex; align-items: flex-start; gap: 0.75rem;">
          <i class="fa-solid fa-circle-check" style="font-size: 1.3rem; margin-top: 2px;"></i>
          <div>
            <strong>Registration Received Successfully!</strong>
            <p style="margin: 0.4rem 0 0.8rem; font-size: 0.88rem; color: #cbd5e1;">
              Thank you, <strong>${nameVal}</strong>. Our counselors have logged your request for <strong>${courseVal}</strong>.
            </p>
            <a href="${whatsappUrl}" target="_blank" rel="noopener noreferrer" class="btn btn-whatsapp btn-sm" style="display: inline-flex;">
              <i class="fa-brands fa-whatsapp"></i> Click to Send Details on WhatsApp
            </a>
          </div>
        </div>
      `;
